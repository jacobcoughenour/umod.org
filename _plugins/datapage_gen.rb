...
module Jekyll
  ...
  class DataPage < Page
    ...
    def initialize(site, base, index_files, dir, data, name, description, template, extension)
      @site = site
      @base = base
      ...
      self.data['description'] = description.nil? ? data[description] : ''
        ...
        data.each do |data_spec|
          ...
          if site.layouts.key? template
            ...
            records.each do |record|
              site.pages << DataPage.new(site, site.source, index_files_for_this_data, dir, record, name, description, template, extension)
            end
          else
            puts "Could not find template #{template}" if not site.layouts.key? template
          end
        end
      end
    end
